...
CREATE TABLE attendance_records (
  ...
  INDEX(user_id, date)  -- 勤怠データの検索を効率化
);
...
CREATE TABLE notifications (
  ...
  deleted_at TIMESTAMP NULL DEFAULT NULL,  -- 論理削除用
  FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,  -- 外部キー制約（usersテーブル参照）
  INDEX(user_id)  -- 通知の検索効率化
);

-- 月次経理データ
CREATE TABLE accounting_reports (
  id INT AUTO_INCREMENT PRIMARY KEY,  -- 経理データID
  year_month VARCHAR(7) NOT NULL,  -- 年月（YYYY-MM）
  total_salary NUMERIC(12,2) DEFAULT 0,  -- 合計給与
  total_expense NUMERIC(12,2) DEFAULT 0,  -- 合計経費
  net_income NUMERIC(12,2) GENERATED ALWAYS AS (total_salary - total_expense) STORED,  -- 自動計算フィールド（収入 - 経費）
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,  -- 作成日時
  deleted_at TIMESTAMP NULL DEFAULT NULL,  -- 論理削除用
  INDEX(year_month)  -- 月次データの検索効率化
);
